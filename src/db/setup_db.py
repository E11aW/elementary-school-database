...
'''
Separated database configuration from code
for the purpose of ease of development. The initial standalone sql
file used absolute paths, which made it a pain for setting up the database
in different environments. 

Also moved config out into a separate file to add flexibility for 
development environments.

Most of debugging was primarily for initial development to make sure that the
database was getting imported correctly. Also used emojis to make it a bit more readable
(and because I just learned that I could do that)
'''
...
def setup_database(config, drop_existing=False):
    ...
    try:
        ...
        print("✅ Connected to PostgreSQL server") # the idea of using emojis was inspired by (shamelessly stolen from) a YT shorts video
        ...
        if drop_existing:
            try:
                ...
                # Disconnect all users from the database just in case
                cursor.execute(f"""
                ...
                # Reconnect to the server and remove database parameter
        ...
        # Setup tables to verify due to possible table mismatch
        tables = [
        ...
        for table in tables:
            ...
            try:
                copy_command = f"COPY {table} FROM '{csv_file}' WITH (FORMAT CSV, HEADER true, DELIMITER ',', NULL '');"
